...
<template>
  ...
  <div class="flex flex-col h-screen w-full justify-center items-center">
    ...
    <DefaultAuthCard class="" :subtitle="`${ SYSTEM_DEVELOPER }`" :title="`Inicia sesión en ${ APP_NAME }`">
      ...
      <span v-if="errors.password" class="text-pink-500">{{ errors.password }}</span>
      
      <div class="mb-5 mt-6">
        <input
        @click="loginUser(userModel.email, userModel.password)"
        type="submit"
